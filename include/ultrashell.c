...
void ultrashell(int n)
{
	...
	do
	{
		    ...
		    if(strEql(ch,"cmd"))
		    {
		            print("\nkernel: You are already in CMD mod\n");
					ultrashell(n+1);
		    }
		    ...
		    else if(strEql(ch,"add"))
		    {
		    	sum();
		    }
		    else if(strEql(ch,"logout"))
		    {
		    	print("\nkernel: system.c: logout\n");
		    }
		    ...
		    {
		            print("\nkernel: error.bin: This command is not exist !\n");
		    } 
	} while (!strEql(ch,"logout"));
}
...
void help()
{
	print("\ncmd       : Starting New UltraShell Interface");
